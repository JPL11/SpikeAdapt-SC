# ============================================================================
# ABLATION: Layer3 SpikeAdapt-SC with CE-only loss
# ============================================================================
# On 4×4: CE-only gained +2.07% → entropy loss HURT
# On 8×8: entropy masking now works → will CE-only still be better?
# ============================================================================
...
SNAP_DIR = "./snapshots_ablation_L3_ce/"
BACKBONE_DIR = "./snapshots_spikeadapt/"
os.makedirs(SNAP_DIR, exist_ok=True)


# ############################################################################
# CORE (identical to train_layer3_split.py)
# ############################################################################
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        print(f"  Feature: {d.shape}")  # (1, 1024, 8, 8)

    spikeadapt = SpikeAdaptSC(C_in=1024, C1=256, C2=128, T=8, eta=0.5, temperature=0.1).to(device)
    print(f"  Params: {sum(p.numel() for p in spikeadapt.parameters()):,}")

    # ==================================================================
    # STEP 2: CE-ONLY (no entropy, no rate loss)
    # ==================================================================
    print("\n" + "="*60)
    print("L3 ABLATION: CE-ONLY (no entropy loss, no rate loss)")
    print("="*60)
    ...
    for ep in range(60):
        ...
        pbar = tqdm(train_loader, desc=f"CE S2 E{ep+1}/60")
        for img, lab in pbar:
            img, lab = img.to(device), lab.to(device)
            ...
            out = back(Fp)
            loss = criterion(out, lab)  # CE ONLY
        ...
        if (ep + 1) % 5 == 0:
            ...
            if acc > best_s2:
                ...
                torch.save(spikeadapt.state_dict(), os.path.join(SNAP_DIR, f"ce_s2_best_{acc:.2f}.pth"))
                print(f"  ✓ Best: {acc:.2f}%")
            else:
                no_improve += 5
    ...
    if s2f:
        spikeadapt.load_state_dict(torch.load(os.path.join(SNAP_DIR, s2f[-1]), map_location=device))

    # ==================================================================
    # STEP 3: Fine-tune head (CE-only)
    # ==================================================================
    print("\n" + "="*60)
    print("STEP 3: Fine-tune (L3 CE-only)")
    print("="*60)
    ...
    for ep in range(40):
        ...
        for step, (img, lab) in enumerate(pbar):
            ...
            ber = random.uniform(0, 0.3)
            with torch.no_grad(): feat = front(img)
            Fp, _, tx, stats = spikeadapt(feat, bit_error_rate=ber)
            loss = criterion(back(Fp), lab) / 2
            loss.backward()
            if (step + 1) % 2 == 0:
        ...
        if (ep + 1) % 5 == 0:
            ...
            if acc > best_s3:
                ...
                torch.save({'spikeadapt': spikeadapt.state_dict(), 'back': back.state_dict()},
    ...
    if s3f:
        ...
        spikeadapt.load_state_dict(ckpt['spikeadapt']); back.load_state_dict(ckpt['back'])

    print("\nBER Sweep (L3 CE-only):")
    ce_results = []
    for ber in [0.0, 0.01, 0.02, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]:
        accs = [evaluate(front, spikeadapt, back, test_loader, ber=ber)[0]
    ...
    print("\n" + "="*60)
    print("COMPARISON: L3 CE-only vs L3 Full Loss")
    print("="*60)
    print(f"{'BER':<8} {'CE-only':<12} {'Full':<12} {'Δ':<8}")
    print("-"*40)
    for r in ce_results:
    ...
    print(f"\nL3 CE S2 Best: {best_s2:.2f}%")
    print(f"L3 CE S3 Best: {best_s3:.2f}%")
    print(f"L3 Full S3 Best: 74.50%")

    with open(os.path.join(SNAP_DIR, "ce_only_L3_results.json"), 'w') as f:
        json.dump({'ce_results': ce_results, 'best_s2': best_s2, 'best_s3': best_s3}, f, indent=2)
    print(f"\n✅ L3 CE-ONLY ABLATION COMPLETE!")
