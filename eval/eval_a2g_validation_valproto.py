...
Output: eval/seed_results/a2g_validation_valproto.json (valproto checkpoints; F2 panel fix)
...
def _best(d):
    c = _glob.glob(d + "/v5cna_best_*.pth")
    return max(c, key=lambda p: float(p.rsplit("_", 1)[1][:-4]))
AID_CK = _best("./snapshots_aidvalp_v5cna_seed42")
R45_CK = _best("./snapshots_resisc45valp_v5cna_seed42")
...
def main():
    ...
    for ds_name, ds_key, n_classes, DsCls, ds_kwargs, bb_path, ck_path in [
        ('AID', 'aid', 30, AIDDataset5050, dict(seed=42),
         './snapshots_aidvalp_5050_seed42/backbone_best.pth',
         AID_CK),
        ('RESISC45', 'resisc45', 45, RESISC45Dataset,
         dict(train_ratio=0.20, seed=42),
         './snapshots_resisc45valp_5050_seed42/backbone_best.pth',
         R45_CK),
    ]:
        print(f'\n===== {ds_name} =====', flush=True)
    ...
    out = 'eval/seed_results/a2g_validation_valproto.json'
