...
SNAP_DIR = "./snapshots_tinyimagenet/"
...
class ResNet50Front(nn.Module):
    """Split at layer3 for both datasets. Output: (1024, H, W)."""
...
if __name__ == "__main__":
    ...
    if os.path.exists(bb_path):
        ...
        print("\n" + "="*60)
        print("STEP 1: Train ResNet50 Backbone on Tiny-ImageNet")
        print("="*60)
        
        all_params = list(front.parameters()) + list(back.parameters())
        opt = optim.SGD(all_params, lr=0.1, momentum=0.9, weight_decay=5e-4)
        sched = optim.lr_scheduler.MultiStepLR(opt, milestones=[30, 60, 80], gamma=0.2)
        criterion = nn.CrossEntropyLoss()
        
        best_bb = 0.0
        for ep in range(100):
            front.train(); back.train()
            pbar = tqdm(train_loader, desc=f"BB E{ep+1}/100")
            for img, lab in pbar:
                img, lab = img.to(device), lab.to(device)
                out = back(front(img))
    ...
    for channel_type in ['bsc', 'awgn', 'rayleigh']:
        ...
        back_state = torch.load(bb_path, map_location=device)
        back.load_state_dict({k: v for k, v in back_state.items()
                              if k.startswith(('layer4.', 'fc.', 'avgpool.'))}, strict=False)
