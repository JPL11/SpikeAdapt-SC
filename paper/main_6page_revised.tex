...
\begin{abstract}
UAV edge intelligence must cope with both unreliable air-to-ground links and stringent onboard power limits. We present \textbf{SpikeAdapt-SC}, a spiking semantic communication system for aerial scene classification that combines a native $14\times14$ spatial grid, learned block masking, and low-rate spike-domain transmission. The key design choice is to avoid coarse pooling: on the native grid, masking becomes fine-grained enough to improve robustness under noise rather than merely save bandwidth. On AID and NWPU-RESISC45, SpikeAdapt-SC reaches 95.42\% and 92.01\% clean accuracy, while maintaining 93.34\% and 86.98\% at BER$=0.30$. At $\rho=0.625$ (37.5\% payload reduction), it matches or exceeds the full-rate no-mask SNN baseline under noise on RESISC45 while remaining competitive on AID. A learned-mask ablation against random and uniform masking confirms that the gain is not explained by pruning alone. Using MPBN, the encoder firing rate drops to 0.167, corresponding to 37.3$\times$ lower estimated compute energy via SynOps than a full-precision MAC-based realization.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Fine-grained native-grid masking.} A BER-conditioned importance scorer selects informative spatial blocks on the native $14\times14$ feature map. At $\rho=0.625$, SpikeAdapt-SC matches or exceeds the full-rate no-mask SNN baseline at BER$=0.30$ on RESISC45 ($+$1.98~pp, seed-42; $+$0.69~pp mean over 5 seeds) while remaining competitive on AID.
    \item \textbf{Robust spike-domain transmission.} The spiking encoder maintains high task accuracy under severe BSC corruption: on AID, accuracy drops from 95.42\% clean to 93.34\% at BER$=0.30$, while an 8-bit CNN baseline falls to 52.80\%.
...
\midrule
\textbf{SpikeAdapt-SC} ($\rho{=}0.75$) & \textbf{94.86}{\scriptsize$\pm$0.63} & \textbf{92.80}{\scriptsize$\pm$0.90} & \textbf{92.52}{\scriptsize$\pm$0.17} & \textbf{85.53}{\scriptsize$\pm$5.48} & 75\% \\
SpikeAdapt-SC ($\rho{=}0.625$)$^\dagger$ & 95.40 & 93.50 & 91.06 & 87.29 & 62.5\% \\
SNN (no mask, $\rho{=}1.0$) & 94.86{\scriptsize$\pm$0.63} & 92.68{\scriptsize$\pm$1.04} & 92.52{\scriptsize$\pm$0.17} & 85.50{\scriptsize$\pm$5.58} & 100\% \\
...
Table~\ref{tab:main_results} shows three key outcomes. First, the spiking pipeline remains robust at BER$=0.30$: on AID, SpikeAdapt-SC drops only 2.08 pp from clean to BER$=0.30$, while CNN-Uni drops 38.98~pp. Second, spatial masking is competitive with or better than full-rate: at $\rho=0.625$ (seed-42), SpikeAdapt-SC exceeds the full-rate no-mask SNN baseline by 1.14~pp on AID and 1.98~pp on RESISC45; a 5-seed paired test confirms this direction on RESISC45 ($\Delta{=}+$0.69~pp, $p{=}0.19$), while on AID the advantage is seed-dependent ($\Delta{=}-$1.76~pp, $p{=}0.03$). Third, a matched-payload non-spiking 1-bit CNN (CNN-1bit) achieves nearly identical clean accuracy but drops to 87.56\% on AID at BER$=0.30$---a 5.78~pp gap vs.\ SpikeAdapt-SC's 93.34\%. This confirms that the robustness advantage is not explained by binary transmission alone; temporal spike coding with $T{=}8$ timesteps provides significantly better noise resilience than single-shot binarization.
