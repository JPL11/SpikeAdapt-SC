...
\begin{enumerate}
    \item \textbf{Fine-grained native-grid masking.} A BER-conditioned importance scorer selects informative spatial blocks on the native $14\times14$ feature map. At $\rho=0.625$, SpikeAdapt-SC exceeds the full-rate no-mask SNN baseline at BER$=0.30$ on both AID and RESISC45.
...
\begin{figure}[t]
    ...
    \caption{Rate--accuracy tradeoff on AID and RESISC45. Under severe noise, $\rho=0.625$ exceeds full-rate $\rho=1.0$ on both datasets while reducing payload by 37.5\%.}
    \label{fig:pareto}
\end{figure}
