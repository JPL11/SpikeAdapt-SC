...
\begin{abstract}
Unmanned aerial vehicles (UAVs) performing edge AI inference face a dual constraint: air-to-ground links suffer from severe fading and limited bandwidth, while onboard processors are restricted by strict power budgets. Current semantic communication systems transmit intermediate neural features uniformly, wasting bandwidth on semantically irrelevant spatial regions and exposing the entire payload to channel noise. We present \textbf{SpikeAdapt-SC}, a spiking neural network (SNN) based semantic communication framework with content-adaptive spatial masking. Our system (i)~encodes deep features as binary spike trains on a native $14{\times}14$ feature grid (196 spatial blocks), yielding an estimated 32--48\% computation energy savings via synaptic operations (SynOps) versus multiply-accumulate operations (MACs); (ii)~employs a learned importance scorer that selectively masks up to 25\% of spatial blocks, \emph{improving} accuracy over full-rate unmasked transmission (96.35\% vs.\ 95.70\%) while saving bandwidth; and (iii)~demonstrates a multi-exit temporal decoder that achieves 96.55\% accuracy at an average of 5.9 out of 8 timesteps, providing an additional 26\% temporal bandwidth reduction. Evaluated on the AID aerial scene benchmark (30 classes, BSC channel), SpikeAdapt-SC maintains $>$92\% accuracy through BER$=$0.30 while a conventional CNN with 8-bit quantization collapses to 67\%. These results establish SNN-based adaptive semantic communication as a viable paradigm for power- and bandwidth-constrained aerial edge intelligence.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Learned spatial importance masking on a fine-grained grid.} A lightweight convolutional scorer evaluates each of the $14{\times}14 = 196$ spatial blocks and selectively masks up to 25\% before transmission, purely from content semantics and without channel state feedback. Every test image receives a unique mask (2,000/2,000), and the learned mask \emph{outperforms} all 100 random draws at $\rho{=}0.75$ by 1.47~pp ($8.8\sigma$). At $\rho{=}0.50$, the gap widens to 10.78~pp.
    
    \item \textbf{Joint spatial--temporal bandwidth reduction.} A multi-exit decoder trained at $T{=}\{2,4,6,8\}$ enables confidence-based temporal early stopping, achieving 96.55\% accuracy at an average of 5.9 timesteps. Combined with 25\% spatial masking, the system achieves $\sim$44\% total bandwidth reduction.
\end{enumerate}

We validate SpikeAdapt-SC on the AID aerial scene classification dataset~\cite{xia2017aid} (30 classes, 10,000 images at $600{\times}600$ resolution) under the BSC channel model. Accuracy denotes top-1 classification accuracy on a 2,000-image test set (80/20 split). Results show $>$92\% accuracy through BER$=$0.30, while CNN-based 8-bit uniform quantization collapses to 67\%.
...
\begin{figure}[t]
    ...
    \caption{SpikeAdapt-SC system architecture. The UAV extracts semantic features via ResNet-50 (L1--L3) and encodes them as spike trains using IF neurons over $T{=}8$ timesteps. A learned scorer masks low-importance blocks on the native $14{\times}14$ spatial grid before A2G transmission. The ground station decodes via a matched SNN decoder and classifies with ResNet-50 L4+FC.}
...
Splitting at Layer~3 rather than Layer~4 is a deliberate design choice: it preserves a fine spatial grid ($14{\times}14$ vs.\ $7{\times}7$), enabling granular content-adaptive masking. On the AID test set, every test image receives a unique mask (2,000 out of 2,000), confirming that spatial granularity is essential for per-image adaptation.
...
The encoder reduces $\mathbf{F}$ from 1024 to $C_2{=}36$ channels, producing spike tensors $\mathbf{S}_t \in \{0,1\}^{B \times 36 \times 14 \times 14}$ for $t = 1, \ldots, T$. Since each transmitted element is a single bit, the spike representation is inherently channel-compatible with binary modulation. We set $C_2{=}36$ to match the per-timestep payload of the 196-block grid ($196 \times 36 = 7{,}056$ values) to the reference 64-block grid with $C_2{=}128$ ($64 \times 128 = 8{,}192$ values), ensuring a fair comparison (see Section~\ref{sec:eval}).
...
\textbf{Reproducibility.} We report results over 5 random seeds (42, 101, 123, 456, 789). Clean accuracy: 96.35, 96.55, 96.05, 95.90, 96.35\% (mean $96.24 \pm 0.25$\%). Table~\ref{tab:main_results} reports the seed-42 model; the tight variance ($\sigma {=} 0.25$~pp) confirms stability.

\textbf{Baselines.} We compare against two baselines: (1)~\emph{SNN (no mask)}: our own SNN encoder/decoder architecture with masking disabled ($\rho{=}1.0$), i.e.\ all blocks transmitted at full rate. This is architecturally similar to SNN-SC~\cite{wang2024snnsc} but trained with our pipeline, so we label it distinctly.\footnote{We do not claim a faithful reproduction of the external SNN-SC system, which differs in architecture, training recipe, and dataset split.} (2)~\emph{CNN-Uni}: a conventional ANN encoder with 8-bit uniform quantization, matched bottleneck dimensions, and the same BSC channel---representing a strong digital baseline.
...
\renewcommand{\arraystretch}{1.15}
\centering
\caption{AID classification accuracy (\%) under BSC channel. Accuracy is top-1 on the 2,000-image test set.}
...
Table~\ref{tab:main_results} presents the headline results on AID (30 aerial scene classes) under BSC channel. We compare SpikeAdapt-SC against two baselines: SNN (no mask)---our architecture with masking disabled ($\rho{=}1.0$, all 196 blocks transmitted), and CNN-Uni (ANN encoder with 8-bit uniform quantization).

\textbf{SpikeAdapt-SC beats full-rate SNN at 75\% bandwidth.} Our masked system (96.35\%; 5-seed mean $96.24 \pm 0.25$\%) outperforms the unmasked baseline (95.70\%) by 0.65~pp while transmitting 25\% fewer bits. Under noise, the advantage persists: +0.50~pp at BER$=$0.15, +0.70~pp at BER$=$0.30.
...
\begin{figure}[t]
    ...
    \caption{Rate--accuracy tradeoff on a representative Airport scene. Red overlay indicates masked (untransmitted) spatial blocks. The system maintains correct classification with high confidence from $\rho{=}1.0$ (196/196 blocks, 100\%) down to $\rho{=}0.5$ (98/196 blocks, 50\% bandwidth). At $\rho{=}0.25$, only 49 blocks are transmitted and the prediction fails---establishing the practical lower bound for bandwidth reduction.}
...
Combined with spatial masking at $\rho{=}0.75$, the total bandwidth reduction is approximately $1 - (0.75 \times 5.9/8) \approx 45\%$ under clean conditions, with accuracy (96.55\%) still exceeding the full-rate unmasked baseline (95.70\%).
...
We presented SpikeAdapt-SC, a spiking neural network framework for semantic communication in aerial networks. By encoding semantic features as binary spike trains and intelligently masking spatial blocks on a native $14{\times}14$ feature grid, the system simultaneously addresses the power and bandwidth constraints of UAV edge AI. Key findings include: (i)~learned spatial masking at $\rho{=}0.75$ achieves 96.35\% accuracy, \emph{outperforming} the unmasked baseline (95.70\%) and CNN-Uni (92.50\%) while saving 25\% bandwidth; (ii)~a randomization analysis confirms the learned mask exceeds all 100 random draws at $\rho{=}0.75$ ($8.8\sigma$; gap widens to 10.78~pp at $\rho{=}0.50$); (iii)~multi-exit temporal decoding provides an additional 26\% temporal savings (avg $T{=}5.9$ at 96.55\% accuracy), yielding $\sim$45\% total bandwidth reduction; and (iv)~binary spike encoding maintains $>$92\% accuracy through BER$=$0.30 while CNN-based quantization collapses to 67\%.
