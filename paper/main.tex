...
\textbf{Baselines.} (1)~\emph{SNN-SC~\cite{wang2024snnsc}}: our SNN with masking disabled ($\rho{=}1.0$), equivalent to the SNN-SC architecture without spatial masking. (2)~\emph{CNN-Uni}: ANN encoder with 8-bit uniform quantization, matched bottleneck. This reflects the fragility of fixed-precision continuous representations under bit errors; comparison against learned quantization (e.g., LSQ~\cite{esser2020lsq}) is left to future work.
...
\renewcommand{\arraystretch}{1.15}
\centering
\caption{Classification accuracy (\%) on AID (50/50 split) under BSC channel. V5C-NA uses MPBN with noise-aware scorer. $^\dagger$5-seed std in parentheses.}
...
Table~\ref{tab:main_results} presents results on AID (standard 50/50 split, 5,000 test images) under the BSC channel, with 5-seed standard deviations from independent scorer retraining. V5C-NA at $\rho{=}0.75$ achieves 95.42$\pm$0.71\% clean accuracy. At $\rho{=}0.625$, accuracy at BER${=}0.30$ is 93.50\%---exceeding the unmasked SNN-SC baseline (92.36\%) by 1.14~pp. While this advantage is modest relative to the seed variance ($\sigma{=}2.58$ at BER$=$0.30), it is observed consistently: the masking advantage (masking $>$ full-rate at BER$=$0.30) holds in 4 of 5 seeds.

\textbf{Binary encoding provides inherent noise robustness.} V5C-NA degrades less than 1~pp from clean to BER${=}0.15$ (95.42\% $\to$ 95.72\%---accuracy improves slightly, consistent with noise acting as regularization). CNN-Uni collapses from 92.50\% to 67.25\% at BER${=}0.30$---a 25.25~pp drop. This validates the fundamental advantage of binary encoding: each spike can only be flipped (bounded distortion), whereas 8-bit values suffer unbounded corruption.
