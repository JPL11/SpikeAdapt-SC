...
Table~\ref{tab:main_results} presents results on AID (standard 50/50 split, 5,000 test images) under the BSC channel, with 5-seed standard deviations from independent scorer retraining. V5C-NA at $\rho{=}0.75$ achieves 95.42$\pm$0.71\% clean accuracy. At $\rho{=}0.625$, accuracy at BER${=}0.30$ is 93.50\%---exceeding the unmasked SNN-SC baseline (92.36\%) by 1.14~pp. While this advantage is modest relative to the seed variance ($\sigma{=}2.58$ at BER$=$0.30), it is observed consistently: the masking advantage holds in 4 of 5 seeds.

\textbf{Binary encoding provides inherent noise robustness.} V5C-NA degrades less than 2~pp from clean to BER${=}0.30$ (95.42\% $\to$ 93.34\%). In contrast, all continuous-valued baselines \emph{collapse}: CNN-Uni drops 39~pp, CNN-NonUni drops 28~pp, and MLP-FC drops 56~pp. MLP-FC degrades faster than CNN baselines because FC layers lack convolutional spatial structure, making them more sensitive to quantization noise. The lower CNN clean accuracy on RESISC45 (e.g., CNN-Uni 78.32\% vs.\ V5C-NA 92.01\%) reflects information loss from 8-bit quantization of the wider 45-class feature space---a fundamental limitation of fixed-precision encoding. JPEG+Conv exhibits the worst behavior---a catastrophic \emph{cliff effect} at BER${=}0.05$, dropping from 95.16\% to 2.68\% (random chance), because even a few residual bit errors after the repetition code corrupt critical JPEG header bytes. This cliff effect is characteristic of separate source-channel coding and motivates joint source-channel approaches. Fig.~\ref{fig:ber_sweep_baselines} shows the full BER sweep for both datasets.
