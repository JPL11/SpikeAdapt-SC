...
\textbf{Baselines.} Following SNN-SC~\cite{wang2024snnsc}, we compare against three baselines using the \emph{same trained backbone}: (1)~\emph{SNN-SC~\cite{wang2024snnsc}}: our SNN with masking disabled ($\rho{=}1.0$), equivalent to the SNN-SC architecture without spatial masking. (2)~\emph{CNN-Uni}: ANN encoder with 8-bit uniform quantization and matched bottleneck, transmitted via bit-plane BSC. (3)~\emph{CNN-NonUni}: same architecture with non-uniform (Lloyd-Max) quantization---k-means on feature activations finds 256~levels that minimize MSE for the actual distribution, then bit-plane BSC. All CNN baselines use the same backbone, encoder/decoder, and noise curriculum as V5C-NA for a fair comparison.
...
\renewcommand{\arraystretch}{1.15}
\centering
\caption{Classification accuracy (\%) on AID (50/50 split) under BSC channel. CNN baselines trained on the same backbone. $^\dagger$5-seed std in parentheses.}
...
\midrule
CNN-Uni (8-bit) & 91.78 & 88.58 & 52.60 & 100\% \\
CNN-NonUni (8-bit) & 91.74 & 89.14 & 62.44 & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Binary encoding provides inherent noise robustness.} V5C-NA degrades less than 2~pp from clean to BER${=}0.30$ (95.42\% $\to$ 93.34\%). In contrast, both CNN baselines \emph{collapse}: CNN-Uni drops 39~pp (91.78\% $\to$ 52.60\%), and CNN-NonUni drops 29~pp (91.74\% $\to$ 62.44\%). Even non-uniform quantization, which optimally adapts quantization levels to the feature distribution, cannot prevent catastrophic degradation because 8-bit representations suffer from MSB (most significant bit) corruption---a single bit flip can change a value by up to $\pm128$, whereas a spike can only flip between 0 and 1. Fig.~\ref{fig:ber_sweep_baselines} shows the full BER sweep for both datasets.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{fig_ber_sweep_baselines.png}
    \caption{BER sweep comparison on (a) AID and (b) RESISC45. Shaded region highlights the SNN noise robustness advantage. Binary spike encoding degrades $<$5~pp at BER${=}0.30$, while CNN 8-bit representations collapse by 29--39~pp.}
...
\textbf{BER robustness.} Table~\ref{tab:ber_sweep} presents the full BER sweep for all methods on both datasets. V5C-NA on AID retains 93.34\% at BER${=}0.30$ (only 2.1~pp drop from clean), and RESISC45 retains 86.98\% (5.0~pp drop). In stark contrast, CNN-Uni collapses from 91.78\% to 52.80\% on AID ($-$39~pp) and from 78.32\% to 49.32\% on RESISC45 ($-$29~pp). CNN-NonUni (Lloyd-Max) is more robust at moderate BER by adapting quantization levels to the feature distribution, but still collapses at BER${=}0.30$ (63.58\% on AID, 50.77\% on RESISC45). The degradation is fundamental: 8-bit representations suffer MSB corruption where a single bit flip changes a value by up to $\pm$128, while a spike can only flip between 0 and 1.

\begin{table}[t]
\renewcommand{\arraystretch}{1.15}
\centering
\caption{Full BER sweep accuracy (\%) on both datasets ($\rho{=}0.75$ for V5C-NA, BSC channel). CNN baselines use the same backbone and data splits.}
\label{tab:ber_sweep}
\begin{tabular}{ll ccccccc}
\toprule
\textbf{Dataset} & \textbf{Method} & \textbf{0.00} & \textbf{0.05} & \textbf{0.10} & \textbf{0.15} & \textbf{0.20} & \textbf{0.25} & \textbf{0.30} \\
\midrule
\multirow{3}{*}{AID} & \textbf{V5C-NA} & \textbf{95.42} & \textbf{95.64} & \textbf{95.70} & \textbf{95.72} & \textbf{95.76} & \textbf{95.16} & \textbf{93.34} \\
 & CNN-Uni & 91.78 & 91.90 & 90.82 & 88.46 & 83.40 & 72.96 & 52.80 \\
 & CNN-NonUni & 91.74 & 91.50 & 90.50 & 89.00 & 85.56 & 78.68 & 63.58 \\
\midrule
\multirow{3}{*}{RESISC45} & \textbf{V5C-NA} & \textbf{92.01} & \textbf{92.15} & \textbf{92.31} & \textbf{92.42} & \textbf{92.31} & \textbf{91.30} & \textbf{86.98} \\
...
\textbf{Key finding: channel-agnostic robustness for both SNN and CNN.} Table~\ref{tab:multichannel} confirms that at matched equivalent BER, BSC, AWGN, and Rayleigh channels produce accuracy within $\pm$0.5~pp for \emph{all} methods---SNN and CNN alike. This is theoretically expected: because both systems transmit binary representations (spikes or bit planes), BPSK hard-decision demodulation converts any additive noise into effective bit flips, reducing all channels to BSC. The critical difference is not channel sensitivity but \emph{noise resilience}: V5C-NA retains 93.34--93.50\% at BER${=}0.30$ across all channels, while CNN-Uni collapses to 49--53\% and CNN-NonUni to 50--65\%. The practical significance is twofold: (i)~binary encoding makes the system inherently channel-agnostic, and (ii)~only SNN encoding provides the bounded-distortion property needed for robust operation at high noise.
...
We presented SpikeAdapt-SC, a spiking semantic communication framework for aerial scene classification. Three findings emerge: (i)~Fine-grained 14$\times$14 spatial masking actively \emph{improves} accuracy under noise---at $\rho{=}0.625$ (37.5\% bandwidth savings), V5C-NA exceeds full-rate accuracy on both AID ($+$1.14~pp) and RESISC45 ($+$1.98~pp). The improvement stems from decoder capacity concentration: masking eliminates noisy low-importance blocks that would otherwise dilute the discriminative signal. (ii)~Binary spike encoding provides inherent, channel-agnostic noise robustness: V5C-NA degrades only 2~pp (AID) and 5~pp (RESISC45) at BER${=}0.30$, while CNN baselines with 8-bit quantization collapse by 29--39~pp under identical conditions---even non-uniform (Lloyd-Max) quantization cannot prevent this catastrophic degradation. At matched equivalent BER, BSC/AWGN/Rayleigh channels produce accuracy within $\pm$0.5~pp for all methods. (iii)~MPBN~\cite{guo2023mpbn} yields $37{\times}$ energy savings via SynOps (FR$=$0.167 vs.\ 0.266). Validation on AID (30 classes, 50/50 split) and RESISC45 (45 classes, 20/80 split) using standard benchmark protocols confirms these benefits.
