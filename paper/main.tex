...
\begin{figure}[t]
    ...
    \caption{BER sweep comparison on (a) AID and (b) RESISC45. V5C-NA ($\rho{=}0.75$), SNN-SC ($\rho{=}1.0$), CNN-Uni, CNN-NonUni, MLP-FC, and JPEG+Conv (AID only, shows cliff). SNN encoding degrades $\leq$5~pp at BER${=}0.30$; 8-bit CNN/MLP baselines collapse by 28--56~pp; JPEG+Conv exhibits a cliff effect at BER${=}0.05$.}
...
\textbf{BER robustness.} Table~\ref{tab:ber_sweep} presents the full BER sweep for all methods on both datasets. V5C-NA on AID retains 93.34\% at BER${=}0.30$ (only 2.1~pp drop from clean), and RESISC45 retains 86.98\% (5.0~pp drop). In stark contrast, CNN-Uni collapses by 39~pp on AID and 29~pp on RESISC45, CNN-NonUni by 28~pp and 27~pp, and MLP-FC by 56~pp and 61~pp. JPEG+Conv suffers a cliff effect at BER${=}0.05$, dropping from 95.16\% to random chance (2.68\%) because even residual errors after repetition decoding corrupt JPEG headers. The degradation hierarchy---JPEG (cliff) $>$ MLP-FC $>$ CNN-Uni $>$ CNN-NonUni $\gg$ SNN---reflects increasing noise resilience from bit-level representations.

\begin{table}[t]
\renewcommand{\arraystretch}{1.15}
\centering
\small
\caption{Full BER sweep accuracy (\%) on both datasets (BSC channel). All baselines share the same backbone and data splits.}
\label{tab:ber_sweep}
...
\midrule
\multirow{4}{*}{RESISC45} & \textbf{V5C-NA} & \textbf{92.01} & \textbf{92.15} & \textbf{92.31} & \textbf{92.42} & \textbf{92.31} & \textbf{91.30} & \textbf{86.98} \\
