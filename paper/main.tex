...
\textbf{BER robustness.} Table~\ref{tab:ber_sweep} presents the full BER sweep for V5C-NA on both datasets. On AID, accuracy at BER${=}0.15$ (95.72\%) exceeds clean (95.42\%), consistent with mild noise acting as regularization. Even at BER${=}0.30$, AID retains 93.34\% (only 2.1~pp drop). RESISC45 follows the same pattern: BER${=}0.15$ (92.42\%) exceeds clean (92.01\%), with a 5.0~pp drop at BER${=}0.30$ to 86.98\%. This broad robustness is attributable to the binary nature of spike encoding: unlike floating-point features, binary spikes can only be flipped, bounding worst-case distortion.
...
\textbf{Key finding: channel-agnostic robustness.} At every matched BER from 0.01 to 0.30, the three channels produce accuracy within $\pm$0.2~pp of each other (Table~\ref{tab:multichannel}, Fig.~\ref{fig:multichannel}). This is theoretically expected: because spikes are binary, BPSK hard-decision demodulation converts any additive noise (Gaussian, fading) into effective bit flips. All three channels therefore \emph{reduce to BSC} at the equivalent BER, meaning the channel model choice is irrelevant for binary-encoded semantic communication. The practical significance is that a system trained on BSC alone deploys without adaptation to AWGN or Rayleigh links---no channel-specific fine-tuning, no CSI feedback.
